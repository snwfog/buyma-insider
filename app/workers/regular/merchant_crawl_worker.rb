  ...
  def perform(merchant_code)
    ...
    @merchant.index_pages.each do |index_page|
      if index_page.cache.fresh? # implies has cache
        logger.info 'Index cache `%s` exists.' % index_page.cache.html_path
        ...
        logger.info 'Index page cache at `%s` is not fresh or does not exists.' % index_page.cache.html_path
