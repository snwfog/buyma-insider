class RedisCleanupWorker < Worker::Base
  ...
  def delete_expired_new_articles(opts, conn)
    ...
    expired_articles_ids = conn.zscan_each(:new_articles_expires_at).take_while { |article|
    ...
    conn.pipelined do
      conn.zrem :new_articles_expires_at, expired_articles_ids
      expired_articles_ids
        .group_by { |article_id| article_id.split(':').first }
        .each_pair do |merchant_code, articles|
          conn.hincrby :new_articles_summary, merchant_code, -articles.count
        end
    end
  end
end